...
start_thread (void *arg)
{
  ...
  /* CONCURRENCY NOTES:
     ...
     In Fil-C, who is responsible for calling __nptl_free_tcb below is
     determined from PD->joinid (a detached thread has PD->joinid == PD)
     instead of from the previous joinstate.  */

  atomic_exchange_relaxed (&pd->joinstate, THREAD_STATE_EXITING);
		      ...
		      void *(*start_routine) (void *), void *arg)
{
  ...
  /* In Fil-C, PD->joinid doubles as the detachment marker (PD for detached
     threads) and tracks joiner claims; see pthread_detach and the exit path
     in start_thread.  */
  ...
  if (__glibc_unlikely (retval != 0))
	...
	/* State (c) and we not have PD ownership (see CONCURRENCY NOTES
	  ...
	  /* Similar to pthread_join, but since thread creation has failed at
	     startup there is no need to handle all the steps.  In Fil-C the
	     kernel never sets PD->joinstate to THREAD_STATE_EXITED (there is
	     no CLONE_CHILD_CLEARTID notification for Fil-C threads), so wait
	     on PD->dead, which the exiting thread sets and wakes itself.  */
	  int dead;
	  while ((dead = atomic_load_acquire (&pd->dead)) == 0)
	    __futex_abstimed_wait64 ((unsigned int *) &pd->dead, dead, 0, NULL,
				     LLL_PRIVATE);
        }
