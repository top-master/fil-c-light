...
#define IMM_STR "0x55"
    ...
    __m128i expected = _mm_gf2p8affineinv_epi64_epi8(a, b, IMM);

    // Legacy SSE form: dest is read/write.
    __m128i sse_result = a;
    asm volatile("gf2p8affineinvqb $" IMM_STR ", %1, %0" : "+x"(sse_result) : "x"(b));
    ...
    __m128i vex_result;
    asm volatile("vgf2p8affineinvqb $" IMM_STR ", %2, %1, %0"
                 : "=x"(vex_result)
                 : "x"(a), "x"(b));
