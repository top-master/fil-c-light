...
__attribute__((__always_inline__)) inline int indexOf(Element key, Element* elements, int count,
...
__attribute__((__noinline__)) int doIndexOf(Element* key, Element* elements, int count,
                                            bool (*compare)(const Element, const Element),
                                            bool isInteger)
{
    return indexOf(*key, elements, count, compare, isInteger);
}
...
int main()
{
    ...
    ZASSERT(doIndexOf(&key, elements, count, compareString, false) == 42);
