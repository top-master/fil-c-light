...
long cpuid_vendor(char* buf);
long rdtsc_now(void);
long rdtscp_now(unsigned* aux);
long xgetbv0(void);
...
int main(void)
{
    ...
    long maxleaf = cpuid_vendor(vendor);
    ...
    long t3 = rdtscp_now(aux);
    unsigned aux1 = *aux;
    long t4 = rdtscp_now(aux);
    unsigned aux2 = *aux;
    if (t4 < t3) {
    ...
    printf("implicit att ok (vendor=%s maxleaf=%ld xcr0=0x%lx)\n", vendor, maxleaf, xcr0);
    return 0;
}
