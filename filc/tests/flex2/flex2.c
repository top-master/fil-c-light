...
{
    struct bar* b = zgc_alloc(__builtin_offsetof(struct bar, z) + 3333 * sizeof(struct foo));
    b->x = 42;
    ...
    for (index = 3333; index--;) {
        b->z[index].x = zgc_alloc(__builtin_offsetof(struct bar, z) + (index % 666) * sizeof(struct foo));
