...
static __attribute__((noinline)) void test1(zweak_map* map)
{
    ...
    while (zweak_map_iter_next(iter)) {
        if (!zweak_map_iter_key(iter)) {
            ...
            ZASSERT(!strcmp(zweak_map_iter_value(iter), "obviate the need for Rust"));
            saw_null = true;
            continue;
        }
...
static __attribute__((noinline)) void doit(void)
{
    ...
    zweak_map_set(map, NULL, strdup("obviate the need for Rust"));
    ZASSERT(!strcmp(zweak_map_get(map, NULL), "obviate the need for Rust"));
    test1(map);
