...
int main()
{
    unlink("filc/test-output/mmapfileb/mmapfile.dat");
    int fd = open("filc/test-output/mmapfileb/mmapfile.dat", O_CREAT | O_RDWR, 0644);
    ASSERT(fd > 2);
    ...
    fd = open("filc/test-output/mmapfileb/mmapfile.dat", O_RDONLY);
    ASSERT(fd > 2);
    char buf[100];
