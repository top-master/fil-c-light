...
/* The .s file exercises the x86_64 `#!` annotation marker: label signature,
   load/store, callsites, a body with a trailing `#` comment, and a full-line
   comment plus .asciz strings containing marker text (none of which may
   fabricate annotations). Sarcasm drops top-level data directives, so the
   strings printed here live in the driver; they match the .s payloads. */
...
int main()
{
    ...
    if (p != &buf[3] || *p != 4) {
        printf("shannot-att BAD\n");
        return 1;
    }
    slot_set(&slot[1], NULL);
    if (slot_get(&slot[1]) != NULL) {
        printf("shannot-att BAD null\n");
        return 1;
    }
