...
long sum5align64_nopush(long a1, long a2, long a3, long a4, long a5);

// Sweep the entry rsp across a full alignment period by C recursion: each of
// 128 levels calls the test and verifies the 1+2+3+4+5 = 15 sum. The incoming
// stack word must read correctly on EVERY mod-64 entry residue; a
...
static void run(int k) {
    ...
    if (k + 1 < 128) run(k + 1);
    long r = sum5align64_nopush(1, 2, 3, 4, 5);
...
int main() {
    run(0);
    printf("and64stackargs-nopush %s\n", ok ? "ok" : "BAD");
    return 0;
}
