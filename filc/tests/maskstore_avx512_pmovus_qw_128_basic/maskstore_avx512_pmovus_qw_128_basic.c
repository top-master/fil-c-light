...
__attribute__((target("avx512vl"))) int main()
{
    ...
    /* Full mask */
    __mmask8 full = 0x03;
    _mm_mask_cvtusepi64_storeu_epi16(buf, full, v);
    for (int i = 0; i < 2; i++)
    ...
    _mm_mask_cvtusepi64_storeu_epi16(buf, partial, v);
    for (int i = 0; i < 2; i++) {
            if (i % 2 == 1)
                ZASSERT(((unsigned short*)buf)[i] == (unsigned short)i);
            else
    ...
    __mmask8 zero = 0;
    _mm_mask_cvtusepi64_storeu_epi16(buf, zero, v);
    for (int i = 0; i < 64; i++)
        ZASSERT(((unsigned char*)buf)[i] == 42);

    zprintf("pmov_qw_128 basic test passed!\n");
    return 0;
}
