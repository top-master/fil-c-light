    ...
    kinds.each {
        ...
        [4, 16].each {
            ...
            if srcKind == :localNaked
                ...
                puts "         ++src_index)"
                puts "        src[src_index] = zasprintf(\"%d\", src_index);"
            end
            puts "    zmemmove_builtin(dst, src, #{size} * sizeof(char*));"
            if dstKind == :localNaked
                size.times {
                    | index |
                    puts "    ZASSERT(!strcmp(dst[#{index}], \"#{index}\"));"
                }
            else
                ...
                puts "         ++dst_index)"
                puts "        ZASSERT(!strcmp(dst[dst_index], zasprintf(\"%d\", dst_index)));"
            end
            puts "}"
            testCount += 1
        }
